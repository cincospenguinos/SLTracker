...
Workout workout_create_today(){
  Workout workout;
  workout.day_type = persist_read_bool(NEXT_DAY_KEY);
  workout.weight[0] = persist_read_int(SQUAT_WEIGHT_KEY);
  
  if(workout.day_type){ // If it's an A day
    workout.weight[1] = persist_read_int(BENCH_WEIGHT_KEY);
    workout.weight[2] = persist_read_int(BENT_WEIGHT_KEY);
  } else { // If it's a B day
    workout.weight[1] = persist_read_int(OVERHEAD_WEIGHT_KEY);
    workout.weight[2] = persist_read_int(DEADLIFT_WEIGHT_KEY);
  }

  time_t temp = time(NULL);
  struct tm *current_time = localtime(&temp);
  
  workout.mday = current_time->tm_mday;
  workout.month = current_time->tm_mon;
  workout.year = current_time->tm_year + 1900; // We need to add on the extra years we have.

  return workout;
}

const char * get_exercise_name(Workout * workout, uint8_t exercise){
  switch(exercise){
  case 0:
    return "Squat";
  case 1:
...
Workout get_workout_at(int index){
  Workout workout;
